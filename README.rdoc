...
= General Object Mapper - Filesystem adapter

The filesystem adapter for the GOM provides an easy way to store object data in the filesystem. Currently the adapter
is read-only. The object data - stored in YML files - can be mapped to object, but no objects can be written to disk.
...
GOM is still under development. Any bug report and contribution is welcome!
